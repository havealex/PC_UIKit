...
RECT CHotKeyWnd::CalPos()
{
    ...
    const int mutiples = 2;
    if (lHeight < rcPos.GetHeight()) {
        rcPos.top += (rcPos.GetHeight() - lHeight) / mutiples;
...
void CHotKeyWnd::OnFinalMessage(HWND)
{
    ...
    if (m_pOwner != nullptr && m_pOwner->m_pWindow != nullptr) {
...
CDuiString CHotKeyWnd::GetKeyName(UINT vk, BOOL fExtended)
{
    ...
    ::GetKeyNameText(nScanCode << offset, szStr, MAX_PATH);
...
CHotKeyUI::CHotKeyUI()
    : m_pWindow(nullptr), m_wVirtualKeyCode(0), m_wModifiers(0), m_uButtonState(0), m_dwHotKeybkColor(0xFFFFFFFF)
{
    const int widthValue = 4;
    const int highValue = 3;
    SetTextPadding(CDuiRect(widthValue, highValue, widthValue, highValue));
...
void CHotKeyUI::SetVisible(bool bVisible)
{
    ...
    if (m_pManager != nullptr && !IsVisible()) {
...
SIZE CHotKeyUI::EstimateSize(SIZE szAvailable)
{
    const int cyStep = 6;
    if (m_cxyFixed.cy == 0) {
        return CDuiSize(m_cxyFixed.cx, m_pManager->GetFontInfo(GetFont())->tm.tmHeight + cyStep);
    }
...
void CHotKeyUI::PaintText(HDC hDC)
{
    ...
    if (IsEnabled()) {
        ...
        CRenderEngine::DrawText(hDC, m_pManager, rc, sText, m_dwDisabledTextColor, m_iFont,
