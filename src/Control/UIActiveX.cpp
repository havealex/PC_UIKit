...
void CActiveXUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    if (m_pUnk != nullptr) {
        ...
        HRESULT hr = GetControl(IID_IWebBrowser2, reinterpret_cast<void **>(&pWebBrowser2));
        if (SUCCEEDED(hr) && (pWebBrowser2 != nullptr)) {
            UINT scaleTemp = CResourceManager::GetInstance()->GetScale();
            float zoomrate = 1;
            if (scaleTemp <= oneScale) {
                zoomrate = 0.7f;
            } else if (scaleTemp <= twoScale) {
                zoomrate = 0.9f;
            } else if (scaleTemp <= thirScale) {
            } else if (scaleTemp <= forScale) {
                zoomrate = 1.4f;
            } else if (scaleTemp <= fivScale) {
                zoomrate = 1.6f;
            } else if (scaleTemp <= sixScale) {
                zoomrate = 1.8f;
            } else {
                zoomrate = 2.0f;
            }
            VARIANT varZoom;
            pWebBrowser2->ExecWB(OLECMDID_OPTICAL_ZOOM, OLECMDEXECOPT_DODEFAULT, nullptr, &varZoom);
            if (V_VT(&varZoom) == VT_I4) {
                UINT iogrizoom = varZoom.lVal;
                std::wstringstream ss;
                if (isUserString(L"ogrizoom")) {
                    CDuiString zoomduistr = getUserString(L"ogrizoom");
                    wstring zoomwst = zoomduistr.GetData();
                    ss << zoomwst;
                    UINT izoom = 0;
                    ss >> izoom;
                    iogrizoom = izoom;
                    ...
                    ss << iogrizoom;
                    CDuiString value = ss.str().c_str();
                    setUserString(L"ogrizoom", value);
                }
                CDuiString zoomvalue = getUserString(L"ogrizoom");
                UINT scalezoom = static_cast<UINT>(iogrizoom * zoomrate);
                LOG_DEBUG(L"CActiveXUI zoom oper  orgizoom:%d  scalezoom:%d zoomrate:%f", iogrizoom, scalezoom,
                    zoomrate);
                VARIANT varZoomIn;          // nZoom是要设置的缩放比例
                varZoomIn.vt = VT_I4;       // /指明整型数据
                varZoomIn.lVal = scalezoom; // /赋值
                pWebBrowser2->ExecWB(OLECMDID_OPTICAL_ZOOM, OLECMDEXECOPT_DODEFAULT, &varZoomIn, nullptr);
            }
        }
    }
