...
namespace DuiLib {
const int ISEP_WIDTH_VALUE = 4;

IMPLEMENT_DUICONTROL(CListExUI)

CListExUI::CListExUI()
    : m_pEditUI(nullptr),
      m_pComboBoxUI(nullptr),
      m_bAddMessageFilter(FALSE),
      m_nRow(-1),
      m_nColum(-1),
      m_pXCallback(nullptr)
{}
...
BOOL CListExUI::CheckColumCheckBoxable(int nColum)
{
    ...
    CListContainerHeaderItemUI *pHItem =
        static_cast<CListContainerHeaderItemUI *>(p->GetInterface(_T("ListContainerHeaderItem")));
    if (pHItem == nullptr) {
        return FALSE;
    }
...
void CListExUI::Notify(TNotifyUI &msg)
{
    ...
    if (_tcsicmp(msg.sType, _T("listheaditemchecked")) == 0) {
        ...
        for (int i = 0; i < GetCount(); ++i) {
            ...
            CListTextExtElementUI *pLItem =
                static_cast<CListTextExtElementUI *>(p->GetInterface(_T("ListTextExElement")));
            if (pLItem != nullptr) {
    ...
    if (_tcsicmp(strName, _T("ListEx_Edit")) == 0 && m_pEditUI && m_nRow >= 0 && m_nColum >= 0) {
        if (_tcsicmp(msg.sType, DUI_MSGTYPE_SETFOCUS) == 0) {
            ...
            CListTextExtElementUI *pRowCtrl = reinterpret_cast<CListTextExtElementUI *>(GetItemAt(m_nRow));
            if (pRowCtrl) {
...
{
    RECT rc = { 0, 0, 0, 0 };
    if (m_pComboBoxUI == nullptr || lpRCColum == nullptr) {
        return;
    }
    if (nColum < 0) {
        if (m_pEditUI) {
...
CListContainerHeaderItemUI::CListContainerHeaderItemUI()
    : m_bDragable(TRUE),
      ...
      m_pOwner(nullptr)
{
    const int txtValue = 2;
    const int minValue = 16;
    SetTextPadding(CDuiRect(txtValue, 0, txtValue, 0));
    ptLastMouse.x = ptLastMouse.y = 0;
    SetMinWidth(minValue);
}
...
{
    const int colorRadix = 16;
    const int txtRadix = 10;
    ...
    if (_tcsicmp(pstrName, _T("dragable")) == 0) {
        ...
        LPTSTR pstr = nullptr;
        DWORD clrColor = _tcstoul(pstrValue, &pstr, colorRadix);
        SetTextColor(clrColor);
    } else if (_tcsicmp(pstrName, _T("textpadding")) == 0) {
        RECT rcTextPadding = { 0 };
        LPTSTR pstr = nullptr;
        rcTextPadding.left = _tcstol(pstrValue, &pstr, txtRadix);
        rcTextPadding.top = _tcstol(pstr + 1, &pstr, txtRadix);
        rcTextPadding.right = _tcstol(pstr + 1, &pstr, txtRadix);
        rcTextPadding.bottom = _tcstol(pstr + 1, &pstr, txtRadix);
...
void CListContainerHeaderItemUI::DoEvent(TEventUI &event)
{
    ...
    if (m_bCheckBoxable) {
        ...
        if (event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK) {
            ...
            if ((m_uCheckBoxState & UISTATE_CAPTURED) != 0) {
                if (::PtInRect(&rcCheckBox, event.ptMouse)) {
                    SetCheck(!GetCheck());
                    CContainerUI *pOwner = reinterpret_cast<CContainerUI *>(m_pParent);
                    if (pOwner) {
                        m_pManager->SendNotify(this, DUI_MSGTYPE_LISTHEADITEMCHECKED, pOwner->GetItemIndex(this),
                            m_bChecked);
                    }
                }
                m_uCheckBoxState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
    ...
    if (event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK) {
        ...
        if (m_iSepWidth >= 0) {
            rcSeparator.left -= stepValue;
        } else {
            rcSeparator.right += stepValue;
        }
...
{
    const int stepValue = 14;
    if (m_cxyFixed.cy == 0) {
        return CDuiSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + stepValue);
    }
...
void CListContainerHeaderItemUI::PaintText(HDC hDC)
{
    ...
    if (m_bShowHtml) {
        ...
        CRenderEngine::DrawText(hDC, m_pManager, rcText, sText, m_dwTextColor, m_iFont, DT_SINGLELINE | m_uTextStyle);
    }
}
...
{
    const int leftStep = 6;
    const int multiples = 2;
    SecureZeroMemory(&rc, sizeof(rc));
    int nItemHeight = m_rcItem.bottom - m_rcItem.top;
    rc.left = m_rcItem.left + leftStep;
    rc.top = m_rcItem.top + (nItemHeight - GetCheckBoxHeight()) / multiples;
...
void CListTextExtElementUI::DoEvent(TEventUI &event)
{
    ...
    // 检查是否需要显示编辑框或者组合框
    const int axesValue = -2; // x,y的值
    if (m_pOwner == nullptr) {
        return;
    }
    CListExUI *pListCtrl = reinterpret_cast<CListExUI *>(m_pOwner);
    ...
    if (event.Type == UIEVENT_BUTTONDOWN && m_pOwner->IsFocused()) {
        ...
        if (nColum >= 0) {
            ...
            ::InflateRect(&rc, axesValue, axesValue);
        }
            ...
            if (event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK) {
                ...
                if ((m_uCheckBoxState & UISTATE_CAPTURED) != 0) {
                    if (::PtInRect(&rcCheckBox, event.ptMouse)) {
                        SetCheck(!GetCheck());
                        if (m_pManager) {
                            m_pManager->SendNotify(this, DUI_MSGTYPE_LISTITEMCHECKED, MAKEWPARAM(GetIndex(), 0),
                                m_bChecked);
                        }
                    }
                    m_uCheckBoxState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
...
SIZE CListTextExtElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    if (cXY.cy == 0 && m_pManager != nullptr && pInfo != nullptr &&
        m_pManager->GetFontInfo(pInfo->nFont) != nullptr) {
        cXY.cy = m_pManager->GetFontInfo(pInfo->nFont)->tm.tmHeight + yStep;
...
void CListTextExtElementUI::DrawItemText(HDC hDC, const RECT &rcItem)
{
    ...
    CListExUI *pListCtrl = reinterpret_cast<CListExUI *>(m_pOwner);
    if (pCallback == nullptr || pListCtrl == nullptr) {
        return;
    }
    m_nLinks = 0;
...
void CListTextExtElementUI::PaintStatusImage(HDC hDC)
{
    ...
    CListExUI *pListCtrl = reinterpret_cast<CListExUI *>(m_pOwner);
    TListInfoUI *pInfo = m_pOwner->GetListInfo();
    if (pListCtrl == nullptr || pInfo == nullptr) {
        return;
    }
    for (int i = 0; i < pInfo->nColumns; i++) {
        if (pListCtrl->CheckColumCheckBoxable(i)) {
