...
void CVerticalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    for (int it2 = 0; it2 < m_items.GetSize(); it2++) {
        ...
        if (iChildAlign == DT_CENTER) {
            int iPosX = (rc.right + rc.left) / TWO_UNIT;
            if (m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible()) {
                iPosX += m_pHorizontalScrollBar->GetScrollRange() / TWO_UNIT;
                iPosX -= m_pHorizontalScrollBar->GetScrollPos();
            }
            RECT rcCtrl = {iPosX - sz.cx / TWO_UNIT, iPosY + rcPadding.top, iPosX + sz.cx - sz.cx / TWO_UNIT,
            ...
            RECT rcCtrl = {iPosX - rcPadding.right - sz.cx, iPosY + rcPadding.top, iPosX - rcPadding.right,
                           iPosY + sz.cy + rcPadding.top};
            pControl->SetPos(rcCtrl, false);
        } else {
            int iPosX = rc.left;
...
RECT CVerticalLayoutUI::GetThumbRect(bool bUseNew) const
{
    if ((m_uButtonState & UISTATE_CAPTURED) != 0 && bUseNew) {
        if (m_iSepHeight >= 0)
            return CDuiRect(m_rcNewPos.left, MAX(m_rcNewPos.bottom - m_iSepHeight, m_rcNewPos.top), m_rcNewPos.right,
                m_rcNewPos.bottom);
        else
...
SIZE CVerticalLayoutUI::EstimateSize(SIZE szAvailable)
{
    ...
    int cyFixed = 0;
    int nEstimateNum = 0;
    SIZE szControlAvailable = { GetFixedWidth(), GetFixedHeight() };
    int iControlMaxWidth = 0;
    int iControlMaxHeight = 0;
    ...
    if (needChangeBySonHeight) {
        return CDuiSize(CResourceManager::GetInstance()->Scale(m_cxyFixed.cx), cyFixed);
    }

    return CContainerUI::EstimateSize(szControlAvailable);
}
}
