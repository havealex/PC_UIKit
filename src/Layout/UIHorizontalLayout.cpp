...
void CHorizontalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    // detemin the layout direction
    bool directionRTL = false;
    switch (UICulture::GetInstance()->getDirection()) {
        case DIRECTION_RTL: // Tonly process rtl
            directionRTL = true;
        default:
            break;
    }
    ...
    auto szItem = m_items.GetSize();
    for (auto it1 = directionRTL ? szItem - 1 : 0; it1 >= 0 && it1 < szItem; directionRTL ? --it1 : ++it1) {
    ...
    if (nAdjustables > 0) {
        int tt = 0;
        int num = nAdjustables;
        cxExpand = MAX(0, (szAvailable.cx - cxFixed) / nAdjustables);
        for (auto pui : tempVec) {
            if (pui->GetMaxWidth()) {
                if (cxExpand > pui->GetMaxWidth()) {
                    tt += cxExpand - pui->GetMaxWidth();
                    num--;
                }
            }
            if (pui->GetMinWidth()) {
                if (cxExpand < pui->GetMinWidth()) {
                    tt -= cxExpand - pui->GetMaxWidth();
                    num--;
                }
            }
        }
        if (num && (num != nAdjustables)) {
            int offset = tt / num;
            cxExpand += offset;
        }
    }
    ...
    for (auto it2 = directionRTL ? szItem - 1 : 0; it2 >= 0 && it2 < szItem; directionRTL ? --it2 : ++it2) {
        ...
        if (szControlAvailable.cx > iControlMaxWidth)
            szControlAvailable.cx = iControlMaxWidth;
        if (szControlAvailable.cy > iControlMaxHeight && (pControl->GetAutoChangeParentHeight() == false))
            szControlAvailable.cy = iControlMaxHeight;
        ...
        if (iChildAlign == DT_VCENTER) {
            int iPosY = (rc.bottom + rc.top) / TWO_UNIT;
            if (m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible()) {
                iPosY += m_pVerticalScrollBar->GetScrollRange() / TWO_UNIT;
                iPosY -= m_pVerticalScrollBar->GetScrollPos();
            }
            RECT rcCtrl = {iPosX + rcPadding.left, iPosY - sz.cy / TWO_UNIT,
                           iPosX + sz.cx + rcPadding.left, iPosY + sz.cy - sz.cy / TWO_UNIT};
            ...
            RECT rcCtrl = {iPosX + rcPadding.left, iPosY - rcPadding.bottom - sz.cy,
                           iPosX + sz.cx + rcPadding.left, iPosY - rcPadding.bottom};
            pControl->SetPos(rcCtrl, false);
        } else {
            int iPosY = rc.top;
            if (m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible()) {
            ...
            RECT rcCtrl = {iPosX + rcPadding.left, iPosY + rcPadding.top, iPosX + sz.cx + rcPadding.left,
...
RECT CHorizontalLayoutUI::GetThumbRect(bool bUseNew) const
{
    if ((m_uButtonState & UISTATE_CAPTURED) != 0 && bUseNew) {
        if (m_iSepWidth >= 0) {
            return CDuiRect(m_rcNewPos.right - m_iSepWidth, m_rcNewPos.top,
                m_rcNewPos.right, m_rcNewPos.bottom);
        } else {
            return CDuiRect(m_rcNewPos.left, m_rcNewPos.top, m_rcNewPos.left - m_iSepWidth,
                m_rcNewPos.bottom);
        }
    } else {
        if (m_iSepWidth >= 0) {
...
SIZE CHorizontalLayoutUI::EstimateSize(SIZE szAvailable)
{
    ...
    bool directionRTL = false;
    switch (UICulture::GetInstance()->getDirection()) {
        case DIRECTION_RTL: // only process rtl
            directionRTL = true;
        default:
            break;
    }
    ...
    int iControlMaxWidth = 0;
    int iControlMaxHeight = 0;
    bool needChangeBySonHeight = false;
    auto szItem = m_items.GetSize();
    for (auto it1 = directionRTL ? szItem - 1 : 0; it1 >= 0 && it1 < szItem; directionRTL ? --it1 : ++it1) {
        CControlUI *pControl = static_cast<CControlUI *>(m_items.GetAt(it1));
        if (pControl == nullptr) {
        ...
        if (!pControl->IsVisible())
            continue;
        if (pControl->IsFloat() && (pControl->GetAutoChangeParentHeight() == false))
            continue;
        szControlAvailable = szAvailable;
        RECT rcPadding = pControl->GetPadding();
        szControlAvailable.cy -= rcPadding.top + rcPadding.bottom;
        ...
        sz.cy = MAX(sz.cy, 0);
        if (pControl->GetAutoChangeParentHeight() && sz.cy < pControl->GetFixedHeight())
        ...
        maxHeight = MAX(cyNeeded, maxHeight);

        nEstimateNum++;

        if (pControl->GetAutoChangeParentHeight() == true) {
            needChangeBySonHeight = true;
        }
    }
    cxFixed += (nEstimateNum - 1) * m_iChildPadding;

    if (needChangeBySonHeight) {
        return CDuiSize(CResourceManager::GetInstance()->Scale(m_cxyFixed.cx), maxHeight);
    }
